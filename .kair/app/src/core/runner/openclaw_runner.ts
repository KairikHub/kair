...
export async function runWithOpenClaw(
  ...
  const apiKey = (
    effectiveProvider === "claude" ? process.env.KAIR_CLAUDE_API_KEY : process.env.KAIR_OPENAI_API_KEY
  || "").trim();
  if (!apiKey) {
    return failed(
  ...
  if (effectiveProvider === "openai" && !String(childEnv.OPENAI_API_KEY || "").trim()) {
    childEnv.OPENAI_API_KEY = apiKey;
  }
  if (effectiveProvider === "claude" && !String(childEnv.ANTHROPIC_API_KEY || "").trim()) {
  ...
  const diagnostics = writeOpenClawDiagnostics({
    ...
    envView: {
      KAIR_OPENAI_API_KEY:
        effectiveProvider === "openai" && apiKey ? "[set]" : effectiveProvider === "openai" ? "[missing]" : "[n/a]",
      KAIR_CLAUDE_API_KEY:
        effectiveProvider === "claude" && apiKey ? "[set]" : effectiveProvider === "claude" ? "[missing]" : "[n/a]",
      OPENAI_API_KEY: String(childEnv.OPENAI_API_KEY || "").trim() ? "[set]" : "[missing]",
      ANTHROPIC_API_KEY: String(childEnv.ANTHROPIC_API_KEY || "").trim() ? "[set]" : "[missing]",
