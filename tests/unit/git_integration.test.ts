...
function setRepoDataEnv(repo: string) {
  ...
  process.env.KAIR_DATA_DIR = path.join(repo, ".contracts");
...
function writeContractFiles(root: string, contractId: string) {
  const contractDir = path.join(root, ".contracts", contractId);
  ...
  fs.writeFileSync(path.join(root, ".contracts", "index.json"), "{ \"contracts\": [] }\n");
}
...
describe("unit: git integration contract-scoped commit paths", () => {
  test("buildContractGitPaths includes plan/governance paths and excludes auth/config + artifacts", () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(repo, ".contracts", contractId, "artifacts", "run", "stream.jsonl"), "{}\n");
      fs.mkdirSync(path.join(repo, ".contracts", contractId, "run"), { recursive: true });
      fs.writeFileSync(path.join(repo, ".contracts", contractId, "run", "stream.jsonl"), "{}\n");
      fs.mkdirSync(path.join(repo, ".contracts", contractId, "git"), { recursive: true });
      fs.writeFileSync(path.join(repo, ".contracts", contractId, "git", "commands.jsonl"), "{}\n");

      const paths = buildContractGitPaths(contractId, repo);
      expect(paths.some((entry) => entry.includes(`.contracts/${contractId}/contract.json`))).toBe(true);
      expect(paths.some((entry) => entry.includes(`.contracts/${contractId}/plan/PLAN.md`))).toBe(true);
      expect(paths.some((entry) => entry.includes(`.contracts/${contractId}/dpc/dpc_v1.json`))).toBe(true);
      expect(paths.some((entry) => entry === ".contracts/index.json")).toBe(true);
      expect(paths.some((entry) => entry.includes(`.contracts/${contractId}/artifacts/`))).toBe(false);
      expect(paths.some((entry) => entry.includes(`.contracts/${contractId}/run/`))).toBe(false);
      expect(paths.some((entry) => entry.includes(`.contracts/${contractId}/git/`))).toBe(false);
  ...
  test("listUncommittedContractPaths detects plan/governance paths and ignores artifacts", () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(repo, ".contracts", contractId, "plan", "extra.txt"), "u\n");
      fs.mkdirSync(path.join(repo, ".contracts", contractId, "artifacts", "run"), { recursive: true });
      fs.writeFileSync(path.join(repo, ".contracts", contractId, "artifacts", "run", "stream.jsonl"), "1\n");
      fs.mkdirSync(path.join(repo, ".contracts", contractId, "run"), { recursive: true });
      fs.writeFileSync(path.join(repo, ".contracts", contractId, "run", "stream.jsonl"), "1\n");
      fs.mkdirSync(path.join(repo, ".contracts", contractId, "git"), { recursive: true });
      fs.writeFileSync(path.join(repo, ".contracts", contractId, "git", "commands.jsonl"), "1\n");

      const dirty = listUncommittedContractPaths(contractId, repo);
      expect(dirty.some((entry) => entry.endsWith(`.contracts/${contractId}/plan/PLAN.md`))).toBe(true);
      expect(dirty.some((entry) => entry.endsWith(`.contracts/${contractId}/plan/extra.txt`))).toBe(true);
      expect(dirty.some((entry) => entry.includes(`.contracts/${contractId}/artifacts/`))).toBe(false);
      expect(dirty.some((entry) => entry.includes(`.contracts/${contractId}/run/`))).toBe(false);
      expect(dirty.some((entry) => entry.includes(`.contracts/${contractId}/git/`))).toBe(false);
  ...
  test("commitPlanChanges commits only contract-scoped .contracts artifacts", () => {
    ...
    try {
      ...
      fs.appendFileSync(path.join(repo, ".contracts", contractId, "plan", "PLAN.md"), "\nnew plan");
      fs.writeFileSync(path.join(repo, ".contracts", contractId, "plan", "generated.md"), "gen\n");
      ...
      expect(lines.some((line) => line.includes(`.contracts/${contractId}/plan/PLAN.md`))).toBe(false);
      expect(lines.some((line) => line.includes(`.contracts/${contractId}/plan/generated.md`))).toBe(false);
    } finally {
      restoreEnv();
      process.chdir(prevCwd);
      fs.rmSync(repo, { recursive: true, force: true });
