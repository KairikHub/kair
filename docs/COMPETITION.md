...
Kair’s distinct position is that governance is the product, not a plugin:
...
- **Weaknesses relative to Kair:**
  ...
  - No native contract/approval object comparable to Kair’s immutable versions and append-only history.
  - Human oversight patterns exist, but responsibility assignment and authority revocation are not the central product model.
- **Why Kair’s approach remains defensible:** Kair can consume any runtime (including SDK-driven ones) while preserving a governance layer where approval and authority are first-class, durable artifacts.
...
- **Why Kair’s approach remains defensible:** Kair treats memory-capable agents as execution substrates while preserving hard governance boundaries that remain stable across changing models and toolchains.
...
- **Why Kair’s approach remains defensible:** Kair can use AutoGen-style systems behind the scenes while keeping governance uniform and explicit at the contract layer.
...
- **Why Kair’s approach remains defensible:** Kair can map onto Temporal-like execution durability while preserving a governance-first domain model that Temporal does not provide natively.
...
- **Why Kair’s approach remains defensible:** Kair captures who authorized what, under which controls, and what superseded what; this goes beyond pipeline run telemetry.
...
- **Why Kair’s approach remains defensible:** Kair can consume policy decisions from engines like OPA while remaining the source of truth for authority contracts and execution accountability.
...
- **Why Kair’s approach remains defensible:** Kair can integrate with these tools for telemetry while preserving governance as a separate, enforceable layer.
...
- **Why Kair’s approach remains defensible:** Kair can adopt or interoperate with these controls as execution-hardening layers while preserving governance as the source of truth.
...
1. **Enterprise policy bridge:** Add optional policy-engine integration (e.g., OPA) so controls can be validated against org policy bundles without diluting Kair’s contract model.
...
Kair should position as the governance authority plane for delegated cognition, not another agent framework:
...
- **“Kair turns AI operations from best-effort trust into enforceable responsibility.”**
...
A realistic framing is that GitHub may implement substantial overlap, and in some environments it may absorb parts of this category. That possibility should be treated as a baseline assumption, not dismissed. Even in that outcome, Kair can still fit as a neutral, local-first governance layer and protocol boundary that is portable across execution backends and platform surfaces. In practice, the durable role is likely coexistence: platforms provide integrated controls, while Kair provides explicit cross-platform responsibility semantics that those platforms can integrate with rather than fully replace.

## Source Snapshots Used

- Kair docs: `README.md`, `docs/ARCHITECTURE.md`, `DECISIONS.md`, `ROADMAP.md`
